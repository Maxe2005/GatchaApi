...
- **`API_generate_gatcha`** protects its sensitive routes (`/admin/*`, `/monsters/generate*`, `/nano-banana/*`, `/external/*`, `/transmission/*`, mutating `/monsters/images/*`) with a Bearer token verified against `API_authentification`'s `POST /user/verify-token` (same contract as the Java `AuthInterceptor`) or an internal API key for machine-to-machine calls, and forwards that bearer token when calling `API_invocations`. It uses **Google Gemini** for both monster stat/text generation (`GEMINI_API_KEY`) and pixel-art image generation — it used to call Banana.dev for images (hence lingering names like `BananaClient`/`BANANA_API_KEY` in older code, since renamed/removed), but that's Gemini too now. Images are produced asynchronously via a Celery worker (Redis-backed) and progress reported over a WebSocket (`/api/v1/monsters/images/ws/{batch_id}`). Generated assets are stored in MinIO (`raw-assets`/`game-assets` buckets). It has an explicit **monster state machine** (`app/models/monster/state.py`, `transition.py`, `update_event.py`) governing generation lifecycle.
